...
public class NavigationSettings extends SettingsPreferenceFragment implements
    ...
    private static final int BUTTONS_ENTRY_VALUE = 2;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        boolean forceBarEnabled = false;

        // if device has capacitive keys and force navigation bar is not enabled,
        // we expose button settings while disabling navigation bar settings
        if (mHasHardwareKeys) {
            forceBarEnabled = isForceNavbarEnabled();
            ...
            PreferenceCategory cat = (PreferenceCategory) findPreference(KEY_NAVBAR_INTERFACE);
            cat.removePreference(mNavbarForce);
            mNavbarForce = null;
        }
        ...
        int targetVal = (mHasHardwareKeys && !forceBarEnabled) ? BUTTONS_ENTRY_VALUE : modeVal;
        updateSettingsTarget(targetVal);
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference.equals(mNavbarMode)) {
            ...
            updateSettingsTarget(!enabled ? BUTTONS_ENTRY_VALUE : Settings.System.getIntForUser(
                    getContentResolver(), Settings.System.NAVIGATION_BAR_MODE, 0, UserHandle.USER_CURRENT));
            return ret;
        }
        return false;
    }
